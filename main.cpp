...
void write_csv(std::string filename, std::vector<std::pair<std::string, std::vector<double>>> transfer_func){
...
\
    ...
    for(int i = 0; i < magnitude.size(); i++){
        ...
        // Wrap into a vector
    std::vector<std::pair<std::string, std::vector<double>>> vals = {{"Frequency", frequencies}, {"Phase", phase}};

    // Write the vector to CSV
    write_csv("phtf2", vals);
